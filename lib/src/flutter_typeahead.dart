...
class TypeAheadValue<T> {
  final String? categoryName;
  final List<T> items;
  TypeAheadValue({this.categoryName, required this.items});
}
...
class TypeAheadFormField<T> extends FormField<String> {
  ...
  TypeAheadFormField(
      ...
      Widget Function(BuildContext, String)? categoryBuilder,
      ...
      required SuggestionsCallback<TypeAheadValue<T>> suggestionsCallback,
...
class TypeAheadField<T> extends StatefulWidget {
  ...
  /// }
  /// ```
  final SuggestionsCallback<TypeAheadValue<T>> suggestionsCallback;
  ...
  final Widget Function(BuildContext, String)? categoryBuilder;
...
class _SuggestionsListState<T> extends State<_SuggestionsList<T>> with SingleTickerProviderStateMixin {
  Iterable<TypeAheadValue<T>>? _suggestions;
  ...
  Future<void> _getSuggestions() async {
    ...
    if (mounted) {
      ...
      Iterable<TypeAheadValue<T>>? suggestions;
      Object? error;
  ...
  Widget createSuggestionsWidget() {
    Widget child = ListView.builder(
      ...
      itemBuilder: (context, index) {
        final TypeAheadValue<T> suggestion = this._suggestions!.elementAt(index);
        return Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            if (suggestion.categoryName != null && widget.categoryBuilder != null)
              widget.categoryBuilder!(context, suggestion.categoryName!),
            ...suggestion.items.map(
              (e) => InkWell(
                child: widget.itemBuilder!(context, e),
                onTap: () => widget.onSuggestionSelected!(e),
              ),
            ),
          ],
        );
